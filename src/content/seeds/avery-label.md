...
title: 'Avery Label Template — Free (5160, 5163, L7160 + Mail Merge Guide)'
h1: 'Avery Label Template'
definitionalLede: 'An Avery label template is a pre-formatted document that matches the exact dimensions and grid of a specific Avery label product code — allowing you to print precisely positioned text or graphics on self-adhesive label sheets using standard word processing or spreadsheet software.'
primaryKeyword: 'avery label template'
searchVolume: 13000
difficulty: 3
...
wordCountFloor: 1800
sources:
  - title: 'Avery — Free templates and software'
    url: 'https://www.avery.com/templates'
    accessed: '2026-04-23'
  - title: 'Microsoft Support — Create and print labels in Word using mail merge'
    url: 'https://support.microsoft.com/en-us/office/create-and-print-labels-using-mail-merge'
    accessed: '2026-04-23'
updated: '2026-04-23'
howTo:
  name: 'How to use an Avery label template'
  steps:
    - name: 'Identify your Avery product code'
      text: 'The product code (e.g., 5160, L7160) is printed on the label packet. US codes are 4–5 digit numbers; UK codes are prefixed with "L". The code determines the number of labels per sheet, their dimensions, and their grid layout. Using the wrong template causes misaligned printing.'
    - name: 'Download the template from avery.com or use Word''s built-in gallery'
      text: 'Go to avery.com/templates and search by product number — Avery provides free .docx and PDF templates. Alternatively, in Microsoft Word, go to Mailings > Labels > Options and select your Avery product from the built-in gallery. In Google Docs, the Avery Label Merge add-on (free) provides the same functionality.'
    - name: 'Enter your data or run a mail merge'
      text: 'For small batches (under 20), type directly into the template cells. For large batches (mailing lists, product labels, name badges), use a mail merge. In Word: Mailings > Start Mail Merge > Labels, connect your Excel or CSV data source, insert merge fields, and preview before printing.'
    - name: 'Print a test sheet on plain paper first'
      text: 'Before loading label sheets, print the template on plain white A4 or Letter paper. Hold the printout against a blank label sheet and hold both up to the light. If the content aligns with the label positions, you are ready to print on the actual label sheet. If not, adjust margins before wasting a sheet.'
    - name: 'Load label sheets face-up and print'
      text: 'Load label sheets in the printer''s manual feed tray where possible, face-up (print side up). Most label sheets are designed for laser printers — if using inkjet, check the label packaging for inkjet compatibility. Let inkjet-printed labels dry for 2–3 minutes before handling to prevent smearing.'
faq:
  - q: 'What is Avery 5160 and how many labels does it have?'
    a: 'Avery 5160 is a US standard address label, 30 labels per sheet (3 columns × 10 rows), each measuring 1" × 2⅝" (25.4mm × 66.7mm). It is the most widely used address label in the US. The UK equivalent is the Avery L7160, which is 21 labels per sheet on A4 (3 columns × 7 rows), each measuring 38.1mm × 63.5mm — slightly different dimensions due to A4 vs Letter paper.'
  - q: 'What is Avery 5163 used for?'
    a: 'Avery 5163 is a US shipping label, 10 labels per sheet (2 columns × 5 rows), each measuring 2" × 4" (50.8mm × 101.6mm). The larger size is designed for shipping labels, where the delivery address, sender address, and any handling instructions need to be clearly readable. The UK equivalent is Avery L7168 (2 per sheet) or L7166 (8 per sheet) depending on the size needed.'
  - q: 'Are there free Avery templates I can download?'
    a: 'Yes. Avery.com provides free .docx and PDF templates for every product code in their range. These are the most reliable source because they are maintained by Avery to match current product specifications. Microsoft Word also has a built-in label template gallery (Mailings > Labels > Options) with Avery codes. Google Docs users can use the free Avery Label Merge add-on from the Google Workspace Marketplace.'
  - q: 'Can I use non-Avery labels with Avery templates?'
    a: 'Yes, as long as the dimensions match. Many own-brand and third-party labels (Herma, Apli, Ryman, Q-Connect in the UK; Office Depot, Staples store brands in the US) produce labels with identical dimensions to common Avery codes. Cross-reference the measurements — label dimensions and grid layout — rather than relying on the product code alone, as codes are not standardised across manufacturers.'
  - q: 'Why is my label printing misaligned?'
  ...
  - q: 'Can I print on a partial sheet of labels?'
...
4. Click inside the first label cell. Go to Mailings > Insert Merge Field. Insert the fields in order: <<First_Name>> <<Last_Name>>, <<Address_Line_1>>, <<Town>>, <<Postcode>>.
...
<aside class="tip">
**Expert tip:** When printing multiple batches on the same day, note the printer's margin offset (if any) on a sticky note attached to the label packet. Future print jobs on the same printer skip the test-print step — you already know the exact adjustment needed. This is particularly useful in an office or parish context where label printing happens monthly and the printer's characteristics are well-known.
</aside>
...
5. Inserts merge fields: <<First_Name>> <<Last_Name>> / <<Address_Line_1>> / <<Address_Line_2>> / <<Town>> / <<Postcode>>.
