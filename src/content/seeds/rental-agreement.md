...
title: 'Rental Agreement Template (US + UK) — Free PDF & DOCX'
h1: 'Rental Agreement Template'
definitionalLede: 'A rental agreement is a contract between a landlord and a tenant for the short-term or month-to-month occupation of residential property, setting out the rent, the deposit, and the responsibilities of each party — and renewing automatically until either side ends it with proper notice.'
primaryKeyword: 'rental agreement template'
searchVolume: 4800
difficulty: 16
...
wordCountFloor: 1800
sources:
  - title: 'gov.uk — Private renting: tenancy agreements'
    url: 'https://www.gov.uk/private-renting/tenancy-agreements'
    accessed: '2026-06-06'
  - title: 'HUD — Rental help and tenant resources'
    url: 'https://www.hud.gov/topics/rental_assistance'
    accessed: '2026-06-06'
  - title: 'California Civil Code §1946 (notice to terminate periodic tenancy)'
    url: 'https://leginfo.legislature.ca.gov/faces/codes_displaySection.xhtml?lawCode=CIV&sectionNum=1946'
    accessed: '2026-06-06'
updated: '2026-06-06'
howTo:
  name: 'How to fill out a rental agreement'
  steps:
    - name: 'Identify the parties and the property'
      text: 'State the full legal names of the landlord and every tenant, and the full address of the rental property, including any unit or room number. Name everyone who will live there.'
    - name: 'Set the rent and the term'
      text: 'State the monthly rent, the day it is due, how it is paid, and whether the agreement is month-to-month or for a short fixed term. For a periodic tenancy, note that it renews automatically each period.'
    - name: 'Record the deposit'
      text: 'State the deposit amount and the conditions for its return. In the UK, the deposit must be protected in a government-approved scheme within 30 days; in many US states, deposit limits and return deadlines are set by statute.'
    - name: 'Set out the rules and responsibilities'
      text: 'Cover who pays which utilities, whether pets and smoking are allowed, maintenance responsibilities, and any house rules. Reference the landlord''s repair obligations, which are largely set by statute and cannot be contracted away.'
    - name: 'State the notice period and sign'
      text: 'State how much notice each party must give to end the tenancy — typically one rental period (e.g. one month) for a month-to-month agreement, subject to statutory minimums. Both landlord and tenant sign and date, and each keeps a copy.'
faq:
  - q: 'What is the difference between a rental agreement and a lease?'
    a: 'A lease is usually a fixed-term contract — six months, a year, or longer — that locks both parties in for the term. A rental agreement is typically month-to-month (a "periodic tenancy"): it renews automatically each month and either party can end it with proper notice. The lease offers stability; the rental agreement offers flexibility. The documents overlap heavily, and some jurisdictions use the terms interchangeably, but the defining difference is fixed term versus rolling, renewable period.'
  - q: 'How much notice is needed to end a month-to-month rental agreement?'
    a: 'It varies by jurisdiction. In the UK, a tenant on a periodic tenancy generally gives at least one month''s notice (or one rental period if rent is paid more often than monthly), while a landlord must give at least two months on a section 21 no-fault notice. In the US it varies by state — California requires 30 days from the tenant and, for tenancies under a year, 30 days from the landlord (60 days for longer occupancy) under Civil Code §1946. Always check the rule for your state or nation; the agreement cannot give less notice than the statutory minimum.'
  - q: 'Does a rental agreement need to be in writing?'
    a: 'A short periodic tenancy can be created verbally and is still legally binding, but a written agreement is strongly advisable because it records the agreed rent, deposit, and rules and prevents disputes. In the UK, tenants are entitled to certain written information regardless. In the US, many states require leases longer than a year to be in writing under the statute of frauds, but month-to-month agreements can be oral — though a written one is always safer for both sides.'
  - q: 'How does the deposit work on a rental agreement?'
    a: 'The deposit (security deposit) protects the landlord against damage and unpaid rent. In the UK, the landlord must protect the deposit in a government-approved tenancy deposit scheme within 30 days and give the tenant the prescribed information — failing to do so blocks the landlord from using a section 21 notice and can mean a penalty of up to three times the deposit. In the US, many states cap the deposit (often one to two months'' rent) and set a deadline (commonly 14–30 days after the tenant leaves) for returning it with an itemised list of any deductions.'
  - q: 'Can a landlord raise the rent on a month-to-month agreement?'
...
builderSchema:
  ...
  sections:
    - id: parties
      ...
      fields:
        - { id: agreement_date, label: 'Date of agreement', type: date, required: true }
        - { id: landlord_name, label: 'Landlord full name', type: text, required: true }
        - { id: tenant_names, label: 'Tenant full name(s)', type: textarea, required: true }
        - { id: property_address, label: 'Property address (incl. unit/room)', type: textarea, required: true }
        - id: jurisdiction
    ...
    - id: term
      ...
      fields:
        - id: term_type
          ...
          required: true
          options:
            - { value: month_to_month, label: 'Month-to-month (periodic)' }
            - { value: short_fixed, label: 'Short fixed term' }
            - { value: room, label: 'Room / lodger' }
        - { id: rent, label: 'Monthly rent', type: currency, required: true }
        - { id: rent_due_day, label: 'Rent due day of month', type: number, required: true }
    ...
    - id: deposit
      heading: 'Deposit & utilities'
      fields:
        - { id: deposit, label: 'Deposit amount', type: currency, required: true }
        - { id: deposit_scheme, label: 'Deposit protection scheme (UK) / holding terms', type: text, required: false }
        - { id: utilities, label: 'Who pays which utilities', type: textarea, required: true }
    - id: rules
      heading: 'Rules & notice'
      fields:
        - { id: pets_allowed, label: 'Pets allowed', type: checkbox, required: false }
        - { id: smoking_allowed, label: 'Smoking allowed', type: checkbox, required: false }
        - { id: house_rules, label: 'House rules', type: textarea, required: false }
...
<aside class="tip">
**Expert tip:** In England and Wales, the deposit rules are a trap that catches even experienced landlords. If you take a deposit, you must protect it in one of the three government-approved schemes within 30 days and serve the tenant the "prescribed information" within the same window. Miss either step and two things happen: you cannot serve a valid section 21 no-fault eviction notice until you put it right, and the tenant can sue you for compensation of one to three times the deposit — even after they have moved out. The fix is simple but the deadline is unforgiving, so protect the deposit and serve the information the moment the tenancy starts, and keep proof of both. This single administrative step is the difference between being able to recover possession and being stuck.
</aside>
...
- **gov.uk — Tenancy agreements** — [gov.uk/private-renting/tenancy-agreements](https://www.gov.uk/private-renting/tenancy-agreements) — the UK government's guidance on tenancy types, deposits, and the documents a landlord must provide.
- **HUD — Rental help** — [hud.gov/topics/rental_assistance](https://www.hud.gov/topics/rental_assistance) — the US Department of Housing and Urban Development's tenant resources and pointers to state landlord-tenant law.
- **California Civil Code §1946** — [leginfo.legislature.ca.gov](https://leginfo.legislature.ca.gov/faces/codes_displaySection.xhtml?lawCode=CIV&sectionNum=1946) — an example of a state statute setting the notice required to end a periodic tenancy.
...
A rental agreement is the flexible cousin of the [lease agreement](/lease-agreement/) — use the lease for a fixed term, the rental agreement for month-to-month, and cross-check the jurisdiction table on the lease page, since the same local rules apply to both. When a tenancy must end against the tenant's wishes, the [eviction notice](/eviction-notice/) template covers the notice-and-court process. A landlord and tenant who agree a repayment plan for rent arrears might document it with a [promissory note](/promissory-note/). A landlord operating through a company will hold the property under an [operating agreement](/operating-agreement/), and a sale of furniture or fittings with the property can be recorded with a [bill of sale](/bill-of-sale/). For billing rent, the [invoice](/invoice-google-docs/) template in the business hub applies.
