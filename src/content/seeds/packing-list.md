...
title: 'Packing List Template — Free Printable by Trip Type (US + UK)'
h1: 'Packing List Template'
definitionalLede: 'A packing list template is a structured checklist of everything to take on a trip, organised by category and tailored to your trip type and climate — so you arrive with what you need and without the three things you always forget.'
primaryKeyword: 'packing list template'
searchVolume: 14000
difficulty: 12
renderer: list-plan
related:
...
sources:
  - title: 'UK FCDO — Foreign travel advice'
    ...
    accessed: '2026-06-06'
  - title: 'US TSA — What can I bring?'
    url: 'https://www.tsa.gov/travel/security-screening/whatcanibring'
    accessed: '2026-06-06'
updated: '2026-06-06'
howTo:
  name: 'How to make a packing list'
  steps:
    - name: 'Start from the trip type and climate'
      text: 'A beach week, a city break, a ski trip, and a business conference need very different lists. Begin by defining the trip — destination, duration, climate, and the activities planned — because everything you pack flows from that. Check the forecast for your dates, not the seasonal average; a "summer" destination can be cold and wet.'
    - name: 'Work through the categories'
      text: 'Go category by category so nothing is forgotten: documents and money, clothing, toiletries, electronics and chargers, medication and health, and trip-specific gear. A category-based list catches the things a free-form list misses — the chargers, the adaptors, the prescription you cannot replace abroad.'
    - name: 'Count the days and apply outfit logic'
      text: 'Plan outfits, not piles. For clothing, count the days, decide how often you will do laundry, and build outfits around a small number of mix-and-match pieces. Most people pack far more than they wear. A capsule of items that combine beats a separate outfit per day.'
    - name: 'Separate carry-on essentials'
      text: 'Mark the items that must go in your carry-on or day bag rather than checked luggage: passport and documents, medication, valuables, a change of clothes, phone and chargers, and anything you cannot afford to lose if a bag is delayed. Check liquid limits for carry-on (100 ml / 3.4 oz containers in a clear bag).'
    - name: 'Check restrictions and check it off'
...
faq:
  - q: 'How do I make a packing list for a specific trip?'
  ...
  - q: 'What should I pack in my carry-on versus checked luggage?'
  ...
  sections:
    - id: trip
      ...
      fields:
        ...
        - { id: dates, label: 'Dates / number of days', type: text, required: true }
        - id: trip_type
          label: 'Trip type'
          type: select
          required: false
          options:
            - { value: beach, label: 'Beach / hot weather' }
            - { value: city, label: 'City break' }
            - { value: ski, label: 'Ski / cold weather' }
            - { value: business, label: 'Business / conference' }
            - { value: camping, label: 'Camping / outdoors' }
            - { value: road_trip, label: 'Road trip' }
            - { value: other, label: 'Other' }
        - { id: climate, label: 'Expected weather (check the forecast)', type: text, required: false }
    - id: items
      heading: 'Items'
      fields:
        ...
        - id: category
          ...
          required: false
          options:
            - { value: documents, label: 'Documents & money' }
            - { value: clothing, label: 'Clothing' }
            - { value: toiletries, label: 'Toiletries' }
            - { value: electronics, label: 'Electronics & chargers' }
            - { value: health, label: 'Health & medication' }
            - { value: gear, label: 'Trip-specific gear' }
            - { value: other, label: 'Other' }
        - { id: quantity, label: 'Quantity', type: text, required: false }
        - { id: carry_on, label: 'Carry-on (not checked)', type: checkbox }
...
<aside class="tip">
**Expert tip:** Build your packing list by reading down your itinerary day by day, not by free-associating about what you "usually" take. For each day, ask one question: what does this day actually require? The hiking day needs boots and a daypack; the formal dinner needs the smart outfit and shoes; the beach afternoon needs swimwear and sun cream; the travel days need comfortable layers and your carry-on essentials. This activity-driven method catches the things a generic list misses — the one occasion that needs the one item you would never have packed otherwise — and it prevents the opposite error of packing for activities you are not actually doing. Your itinerary already contains your packing list; you just have to read it as one.
</aside>
...
A packing list works hand in hand with the other trip-planning documents in this hub. For a driving holiday, the [road trip itinerary](/road-trip-itinerary/) defines the route and the days the packing list must cover (and the road-trip kit it must include). A [meal planner](/meal-planner/) is the companion to a packing list for camping or self-catering trips, where you pack the food and the means to cook it. A [household budget](/household-budget/) frames the trip cost, and a [monthly calendar](/monthly-calendar/) helps you count down to departure and the deadlines (renewing a passport, buying gear) before it. For the day-by-day plan that tells you what each day requires — and therefore what to pack — pair the packing list with the broader [travel itinerary](/travel-itinerary/); the two together cover what you''ll do and what you need to do it.
