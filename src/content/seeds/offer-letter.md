...
title: 'Job Offer Letter Template (US + UK) — Free PDF & DOCX'
h1: 'Job Offer Letter Template'
definitionalLede: 'A job offer letter is a written communication from an employer to a successful candidate setting out the role, salary, start date, and key conditions of employment, which becomes a binding contract once the candidate accepts.'
primaryKeyword: 'job offer letter template'
searchVolume: 4000
difficulty: 14
renderer: letter
related:
...
wordCountFloor: 1800
sources:
  - title: 'Acas — Job offers'
    url: 'https://www.acas.org.uk/job-offers'
    accessed: '2026-06-06'
  - title: 'gov.uk — Employment contracts and the written statement of particulars'
...
updated: '2026-06-06'
howTo:
  name: 'How to write a job offer letter'
  steps:
    - name: 'Name the role and the candidate'
      text: 'Open by congratulating the candidate and stating the exact job title being offered. Ambiguity about the role offered is the most common source of post-offer disputes.'
    - name: 'State the compensation precisely'
      text: 'Give the salary or hourly rate, the pay frequency, and any bonus, commission, or equity. Be explicit about whether figures are gross (UK) or before tax, and whether a bonus is discretionary or contractual.'
    - name: 'Set the start date and key conditions'
      text: 'Name the proposed start date, the location or remote arrangement, the hours, and any conditions the offer is subject to — references, right-to-work checks, a background check, or a medical.'
    - name: 'Add the employment-basis clause'
      text: 'In the US, state clearly that employment is at-will. In the UK, note that this letter is an offer that becomes binding on acceptance and that a full contract of employment (or written statement of particulars) will follow.'
    - name: 'Request acceptance in writing'
      text: 'Ask the candidate to sign and return the letter, or reply confirming acceptance, by a stated deadline. Include the lines for signature and date.'
faq:
  - q: 'Is a job offer letter legally binding?'
    a: 'In the UK, yes — once the candidate accepts an unconditional offer, a contract of employment exists, even before the formal contract document is signed. The offer letter''s terms are binding and a later contract cannot lawfully reduce what the offer promised. In the US, an offer letter is generally not a binding employment contract because employment is at-will; it sets expectations but either party can usually end the relationship at any time. Well-drafted US offer letters say this explicitly to avoid creating an implied contract.'
  - q: 'What is the difference between an offer letter and an employment contract?'
    a: 'An offer letter is the initial, usually shorter document that sets out the headline terms — role, pay, start date — and invites acceptance. An employment contract (in the UK, often combined with the statutory written statement of particulars) is the fuller document covering notice periods, confidentiality, holiday, pension, and policies. In the UK the offer letter, once accepted, is itself part of the contract; the formal contract elaborates rather than replaces it.'
  - q: 'Can an employer withdraw a job offer after it has been made?'
    a: 'It depends on whether the offer was conditional. A conditional offer (subject to references, right-to-work, or a background check) can be withdrawn if a condition is not met. Withdrawing an unconditional offer that has been accepted is, in the UK, a breach of contract — the employer may owe the candidate notice pay. In the US at-will context, an accepted offer can usually be withdrawn before the start date, but doing so after the candidate has resigned from another job can expose the employer to a promissory-estoppel claim.'
  - q: 'What does "at-will" mean in a US offer letter?'
  ...
  sections:
    - id: parties
      ...
      fields:
        ...
        - { id: date, label: 'Date of offer', type: date, required: true }
    - id: role
      heading: 'Role & compensation'
      fields:
        - { id: job_title, label: 'Job title', type: text, required: true }
        - { id: salary, label: 'Salary (gross) or hourly rate', type: text, required: true }
        - id: pay_frequency
          label: 'Pay frequency'
          type: select
          required: true
        ...
        - { id: location, label: 'Location / remote arrangement', type: text, required: true }
        - { id: probation, label: 'Probationary period (optional)', type: text, required: false }
    - id: terms
      heading: 'Conditions & basis'
      fields:
        - id: employment_basis
          ...
          required: true
          options:
            - { value: at_will_us, label: 'At-will (US)' }
            - { value: binding_uk, label: 'Binding on acceptance (UK)' }
        - { id: conditions, label: 'Conditions (references, right-to-work, background check)', type: textarea, required: false }
        - { id: acceptance_deadline, label: 'Acceptance deadline', type: date, required: true }
---
...
<aside class="tip">
**Expert tip:** Keep discretionary and contractual pay rigorously separate in the offer letter. The most expensive offer-letter dispute is over a bonus the employer thought was discretionary and the candidate read as guaranteed. If a bonus is discretionary, say so in those words and explain what it depends on ("a discretionary annual bonus of up to 10% of salary, subject to company and individual performance and at the company's discretion"). If a payment is genuinely guaranteed, call it a contractual bonus and accept that you must pay it. The word "up to" and the word "discretionary" are doing real legal work — in the UK especially, a poorly worded bonus clause has cost employers more than the bonus itself.
</aside>
...
The offer letter is one bookend of the employment relationship; the [resignation letter](/resignation-letter/) and [two weeks notice](/two-weeks-notice/) templates are the other. When a candidate or employer needs a reference, the [reference letter](/reference-letter/) and [letter of recommendation](/letter-of-recommendation/) templates apply. If you are an LLC making your first hires, the [operating agreement](/operating-agreement/) template sets out the governance of the entity doing the hiring. And the [cover letter](/cover-letter-google-docs/) in the careers hub is the document that opened the conversation this offer concludes.
