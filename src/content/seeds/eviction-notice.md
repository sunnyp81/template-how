...
title: 'Eviction Notice Template — Free (UK + US)'
h1: 'Eviction Notice Template'
definitionalLede: 'An eviction notice is a formal written notice from a landlord to a tenant requiring them to vacate a property, remedy a breach of the tenancy agreement, or face legal proceedings to recover possession.'
primaryKeyword: 'eviction notice template'
searchVolume: 4700
difficulty: 13
renderer: letter
related:
...
faq:
  ...
  - q: 'What is a Cure-or-Quit notice?'
    ...
    - id: notice_type
      ...
      fields:
        - id: notice_type
          ...
          required: true
          options:
            - { value: s21, label: 'UK — Section 21 (no-fault)' }
            - { value: s8, label: 'UK — Section 8 (fault-based)' }
            - { value: pay_quit, label: 'US — Pay-or-Quit' }
            - { value: cure_quit, label: 'US — Cure-or-Quit' }
            - { value: unconditional, label: 'US — Unconditional Quit' }
            - { value: no_cause, label: 'US — 30/60-day no-cause notice' }
        - { id: reason, label: 'Reason for notice (required for fault-based notices)', type: textarea, required: false }
        - { id: notice_period, label: 'Notice period (e.g. 2 months, 14 days)', type: text, required: true }
    - id: dates
      heading: 'Dates and service'
      fields:
        - { id: date_served, label: 'Date notice served', type: date, required: true }
        - { id: vacate_by, label: 'Date tenant must vacate by', type: date, required: true }
        - id: service_method
          label: 'Service method'
          type: select
          required: true
          options:
            - { value: first_class, label: 'First-class post' }
            - { value: registered, label: 'Registered post' }
            - { value: hand, label: 'Hand delivery' }
            - { value: combined, label: 'First-class post + hand delivery' }
    - id: signatures
      heading: 'Signature'
      fields:
...
<aside class="tip">
**Expert tip:** UK landlords: before serving any eviction notice, run through the NRLA (National Residential Landlords Association) preconditions checklist. The NRLA publishes a current version and it is updated when legislation changes. Section 21 notices are invalidated by procedural defects that could have been caught with a 10-minute check — the most common is an expired Gas Safety Certificate. Renewing the certificate and re-serving is a straightforward fix if caught early; discovering it halfway through a court case is considerably more disruptive.
</aside>
