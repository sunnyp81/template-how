...
title: 'Estimate Template (US Estimate / UK Quote) — Free PDF & XLSX'
h1: 'Estimate Template'
definitionalLede: 'An estimate (called a quote or quotation in the UK) is a document a seller gives a prospective customer setting out the expected cost of goods or services before the work is agreed — an approximate, non-binding price for an estimate, or a fixed price for a quote.'
...
wordCountFloor: 1800
sources:
  - title: 'gov.uk — Invoicing and taking payment from customers (quotes and estimates)'
    url: 'https://www.gov.uk/invoicing-and-taking-payment-from-customers'
    accessed: '2026-06-06'
  - title: 'SBA — Manage your finances'
    url: 'https://www.sba.gov/business-guide/manage-your-business/manage-your-finances'
    accessed: '2026-06-06'
  - title: 'Citizens Advice — Getting a quote or estimate for work'
    url: 'https://www.citizensadvice.org.uk/consumer/somebody-doing-work-for-you/getting-a-quote-or-estimate/'
    accessed: '2026-06-06'
updated: '2026-06-06'
howTo:
  name: 'How to write an estimate'
  steps:
    - name: 'Label it clearly as an estimate or a quote'
      text: 'Decide whether you are giving an approximate, non-binding estimate or a fixed-price quote, and label the document accordingly. The label affects whether the customer can hold you to the price, so it matters legally.'
    - name: 'Identify your business and the customer'
      text: 'Add your business name, address, and contact details, the customer''s name, and a unique estimate number and date so both sides can reference it.'
    - name: 'Break down the work into line items'
      text: 'List each item of work or each product, with a description, quantity, unit price, and line total. Itemising builds trust and makes it clear what is and is not included.'
    - name: 'Add totals, tax, and exclusions'
      text: 'Show the subtotal, any tax (VAT in the UK, sales tax in the US), and the total. State clearly what is excluded and any assumptions, so the customer is not surprised by extras.'
    - name: 'State validity and how to accept'
      text: 'Say how long the estimate is valid (for example, 30 days) and how the customer accepts — by signing, replying, or issuing a purchase order. A validity period protects you against rising costs.'
faq:
  - q: 'What is the difference between an estimate and a quote?'
    a: 'An estimate is an approximate, best-guess price that can change once the work is properly scoped — it is non-binding. A quote (or quotation) is a fixed price the seller commits to: once the customer accepts it, the seller is generally bound to do the work for that amount. In the US, "estimate" is the common term and is often understood as approximate; in the UK, "quote" usually means a fixed price and "estimate" means an approximate one. The label you use carries legal weight, so choose it deliberately.'
  - q: 'Is an estimate legally binding?'
    a: 'Generally no — an estimate is an approximate figure and the final cost can reasonably differ. A quote, by contrast, is usually binding once accepted, because the seller has committed to a fixed price. That said, even an estimate cannot rise without limit: UK guidance and consumer law expect the final price to be reasonably close to a genuine estimate, and a final bill far above the estimate can be challenged. Always be clear which one you are giving.'
  - q: 'Why does the US say "estimate" and the UK say "quote"?'
    a: 'It is mostly terminology. American businesses — especially in trades like construction, auto repair, and home services — routinely give an "estimate," and customers expect the final invoice to be close to it. British businesses more often give a "quote" or "quotation," understood as a fixed price, and reserve "estimate" for genuinely approximate figures. The underlying document is the same; the word signals how firm the price is. This template serves both markets and lets you label it either way.'
  - q: 'How long should an estimate be valid for?'
    a: 'State a validity period — 14 or 30 days is common — so you are not held to a price after your own costs have changed. Materials prices, supplier rates, and your availability all move over time, and an open-ended estimate exposes you to honouring a figure that no longer covers your costs. A clear "valid for 30 days from the date above" protects you and prompts the customer to decide.'
  - q: 'Does an estimate need to include VAT or sales tax?'
    a: 'It should make the tax position clear. In the UK, if you are VAT-registered, show whether the prices include or exclude VAT and at what rate — a customer who is not VAT-registered cares about the gross figure. In the US, sales tax depends on the state and the nature of the goods or services; show the expected tax or note that it will be added. Hiding tax until the invoice is a common cause of disputes, so state it on the estimate.'
  - q: 'How is an estimate different from an invoice?'
    a: 'An estimate is given before the work, to set the expected price and win the job. An invoice is issued after the work, to request payment for what was actually done. The estimate is a sales document; the invoice is an accounting document. A well-run job flows estimate to acceptance to work to invoice, with the invoice often referencing the original estimate so the customer can see the agreed price carried through.'
  - q: 'Can I charge more than my estimate?'
    a: 'For a genuine estimate, yes — within reason — if the actual work turns out to be more than expected, but you should tell the customer as soon as you realise and ideally get their agreement before incurring the extra cost. For a fixed quote, no — you are bound to the quoted price unless the customer agrees to a variation. UK consumer guidance is clear that a final bill should not be substantially more than an estimate without good reason and notice. Document any agreed changes in writing.'
  - q: 'What should I do if the customer accepts my estimate?'
    ...
    - id: header
      ...
      fields:
        - id: document_label
          ...
          required: true
          options:
            - { value: estimate, label: 'Estimate (approximate — US)' }
            - { value: quote, label: 'Quote / quotation (fixed — UK)' }
        - { id: estimate_number, label: 'Estimate / quote number', type: text, required: true }
        - { id: date, label: 'Date', type: date, required: true }
        - { id: business_name, label: 'Your business name', type: text, required: true }
        - { id: business_contact, label: 'Your address & contact', type: textarea, required: true }
        - { id: customer_name, label: 'Customer name', type: text, required: true }
        - { id: customer_contact, label: 'Customer address & contact', type: textarea, required: false }
    - id: items
      heading: 'Work / items'
      fields:
        - { id: line_items, label: 'Line items (description, qty, unit price)', type: lineitems, required: true }
    - id: totals
      heading: 'Totals & tax'
      fields:
        ...
        - { id: total, label: 'Total', type: currency, required: true }
    - id: terms
      heading: 'Validity & terms'
      fields:
        - { id: valid_until, label: 'Valid until', type: date, required: false }
        - { id: exclusions, label: 'Exclusions & assumptions', type: textarea, required: false }
...
<aside class="tip">
**Expert tip:** Be precise about the difference between an estimate and a quote, because it has real legal consequences. If you label a document a "quote" and the customer accepts it, you are generally bound to do the work for that fixed price — even if it turns out to be more work than you expected. If you genuinely cannot commit to a fixed price because the scope is uncertain, give a clearly labelled "estimate" instead, state the assumptions it rests on, and tell the customer the moment you realise the cost will exceed it. UK consumer guidance expects a final bill not to be substantially more than an estimate without good reason and notice. The cheapest way to avoid a fee dispute is to choose the right word at the top of the document and to flag overruns early, in writing, before the extra cost is incurred.
</aside>
...
> **QUOTATION**   No: Q-2026-031   Date: 6 June 2026
> **From:** Pennine Joinery, 14 Calder Road, Leeds LS10 2BX
> **To:** Mrs A. Okafor, 8 Beech Grove, Leeds LS6 4DT
>
> | Description | Qty | Unit price | Line total |
> |---|---|---|---|
> | Design and survey | 1 | £120.00 | £120.00 |
> | Built-in wardrobe units (MDF, painted) | 3 | £680.00 | £2,040.00 |
> | Soft-close fittings and rails | 1 | £180.00 | £180.00 |
> | Fitting labour (3 days) | 3 | £240.00 | £720.00 |
>
> Subtotal: £3,060.00 · VAT (20%): £612.00 · **Total: £3,672.00**
> **Excludes:** making good and decoration of surrounding walls; electrical work.
> Valid for 30 days. To accept, please sign and return, or confirm by email.
...
- **gov.uk — Invoicing and taking payment from customers** — [gov.uk/invoicing-and-taking-payment-from-customers](https://www.gov.uk/invoicing-and-taking-payment-from-customers) — the UK government's guidance distinguishing quotes from estimates and the documents in the order-to-payment cycle.
- **Citizens Advice — Getting a quote or estimate for work** — [citizensadvice.org.uk](https://www.citizensadvice.org.uk/consumer/somebody-doing-work-for-you/getting-a-quote-or-estimate/) — the consumer-side explanation of how binding a quote is versus an estimate, and what to do if a bill exceeds an estimate.
- **SBA — Manage your finances** — [sba.gov/business-guide/manage-your-business/manage-your-finances](https://www.sba.gov/business-guide/manage-your-business/manage-your-finances) — US Small Business Administration guidance on pricing and financial management.
...
An accepted estimate often leads the buyer to issue a [purchase order](/purchase-order/), and the job ends with an [invoice](/invoice-google-docs/) that references the agreed estimate. The costs you build into an estimate connect to your [expense report](/expense-report/) and financial records. When you win a notable contract, the [press release](/press-release/) template handles the announcement, and pricing decisions taken by a team are recorded in [meeting minutes](/meeting-minutes/). The business issuing estimates is governed by its [operating agreement](/operating-agreement/) in the legal hub.
